...
* 서비스(Service): 노드 간의 동기식 통신을 위한 통신 채널입니다. 서비스는 요청(request)과 응답(response) 방식으로 작동하며, 특정 작업을 요청하고 그 결과를 받을 수 있는 메커니즘입니다. 예를 들어, "move_arm" 서비스는 사용자가 로봇팔의 특정 위치로 이동하도록 요청(request)하고, 그 결과(response)로 이동이 완료되었는지 여부를 반환합니다. 이 방식은 로봇팔이 명령을 정확하게 수행하고 응답할 수 있도록 동기식으로 작동합니다.
