...
* 노드(Node): ROS 시스템에서 기본적으로 실행되는 프로세스 단위입니다. 각 노드는 **독립적**으로 실행되며, 로봇의 다양한 기능을 담당합니다. 예를 들어, "arm_controller"라는 노드는 로봇의 각 joint의 위치를 받아 움직임을 제어하고, "vision_processing"이라는 노드는 카메라 데이터를 받아 물체의 위치를 추적할 수 있습니다. 이 두 노드는 서로 독립적으로 실행되며 필요한 데이터를 주고받습니다.

    ![alt text](/images/robotics/basic/image.png)
...
* 토픽(Topic): 노드 간에 비동기식 통신을 위한 통신 채널입니다. Publisher가 데이터를 특정 토픽에 송신하면, Subscriber는 해당 토픽을 구독하여 데이터를 수신합니다. 예를 들어, "arm_controller" 노드는 "joint_angles"라는 토픽을 통해 현재 각 관절의 위치 정보를 보내고, "vision_processing" 노드는 "detected_object"라는 토픽을 통해 인식한 물체의 위치 정보를 송신할 수 있습니다. 로봇팔 제어 시스템은 이 정보를 통해 물체를 집는 동작을 수행할 수 있습니다.
    ![alt text](/images/robotics/basic/image-1.png)

* 서비스(Service): 노드 간의 동기식 통신을 위한 통신 채널입니다. 서비스는 요청(request)과 응답(response) 방식으로 작동하며, 특정 작업을 요청하고 그 결과를 받을 수 있는 메커니즘입니다. topic과는 다르게 1:1 통신에서만 동작합니다. 예를 들어, "move_arm" 서비스는 사용자가 로봇팔의 특정 위치로 이동하도록 요청(request)하고, 그 결과(response)로 이동이 완료되었는지 여부를 반환합니다. 이 방식은 로봇팔이 명령을 정확하게 수행하고 응답할 수 있도록 동기식으로 작동합니다.

    ![alt text](/images/robotics/basic/image-2.png)
...
* 액션(Action): 시간이 걸리는 작업(예: 로봇의 이동 명령)을 수행할 때 사용되는 개념입니다. 요청을 보낸 후 작업이 완료될 때까지 진행 상황을 모니터링하거나 중간에 취소할 수 있습니다. 예를 들어, "pick_and_place" 액션은 물체를 잡는 동작을 요청하고, 이 과정에서 로봇팔의 이동 경로, 물체를 잡았는지 여부 등을 지속적으로 피드백하며 완료되기까지 상태를 모니터링할 수 있습니다.
    ...
    ![alt text](../images/robotics/basic/image-3.png)

    | **구분**           | **토픽(Topic)**   | **서비스(Service)**                | **액션(Action)**                                    |
    |-------------------|-------------------|-----------------------------------|------------------------------------------------------|
    | **기능**           | 노드 간 비동기적으로 메시지를 주고받는 통신 방식 | 클라이언트가 서버에 요청하고 즉시 응답을 받는 동기식 통신  | 시간이 걸리는 작업을 비동기적으로 처리하고 피드백 및 취소 가능  |
    | **방향성**         | 단방향            | 양방향            | 양방향 |
    | **통신 방식**      | 비동기식 (퍼블리셔와 구독자 간의 통신)   | 동기식 (즉시 응답 대기) | 비동기식 (피드백 및 취소 가능) |
    | **다자간 연결**    | 1:1, 1:N, N:1, N:N (Publisher:Subscriber)   | 1:1 (Server:Client)  | 1:1 (Server:Client) |
    | **작업 예시**      | 실시간 데이터 전송 (예: 센서 데이터 스트리밍) | 즉시 완료 가능한 작업 (예: 로봇팔 관절 제어) | 복잡하고 시간이 걸리는 작업 (예: 물체 집기 및 이동)    |
    | **적합한 작업 유형**| 실시간 연속적인 데이터 전송이 필요한 작업 | 짧고 빠르게 완료되는 작업 | 장시간 작업이나 상태 피드백이 필요한 작업 |
    | **피드백 제공**    | 없음 | 없음 | 작업 진행 중 피드백 가능 |
    | **작업 중단**      | N/A | 중단 불가능 | 작업 중단 및 취소 가능 |
    | **통신 대상**      | N:M (다수의 publisher와 다수의 subscriber) | 1:1 (클라이언트와 서버) | 1:1 (클라이언트와 서버) |
    | **주 사용 시나리오**| 센서 데이터 수집 및 처리, 실시간 정보 전파 | 특정 동작을 요청하고 즉시 결과가 필요한 상황 | 시간이 걸리며 피드백이나 중단이 필요한 상황 |
    | **통신 관계** | 퍼블리셔와 다수의 구독자 간의 1:N 통신 | 클라이언트와 서버 간 1:1 통신 | 클라이언트와 서버 간 1:1 통신 (피드백 가능) |
...
| 용어     | 정의 | 사용 예 |
|----------|----------|----------|
| **Node**     | ROS 시스템에서 기본적으로 실행되는 프로세스 단위|"arm_controller"라는 노드는 각 관절의 위치를 받아 움직임을 제어하고, "vision_processing"이라는 노드는 카메라 데이터를 받아 물체의 위치를 추적할 수 한다.     |
| **Topic**    | 노드 간에 **비동기식 통신**을 위한 통신 채널 |"arm_controller" 노드는 "joint_angles"라는 토픽을 통해 현재 각 관절의 위치 정보를 보내고, "vision_processing" 노드는 "detected_object"라는 토픽을 통해 인식한 물체의 위치 정보를 송신한다.      |
| **Service**  | 노드 간의 **동기식 통신**을 위한 통신 채널 |"move_arm" 서비스는 사용자가 로봇팔의 특정 위치로 이동하도록 요청(request)하고, 그 결과(response)로 이동이 완료되었는지 여부를 반환한다.     |
| **Action**   | 시간이 걸리는 작업(예: 로봇의 이동 명령)을 수행할 때 사용되는 개념. 요청을 보낸 후 작업이 완료될 때까지 진행 상황을 모니터링하거나 중간에 취소할 수 있다. | "pick_and_place" 액션은 물체를 잡는 동작을 요청하고, 이 과정에서 로봇팔의 이동 경로, 물체를 잡았는지 여부 등을 지속적으로 피드백하며 완료되기까지 상태를 모니터링한다.     |
| **Message**  | 노드 간 통신에 사용되는 데이터 구조 |"JointAngles.msg"라는 메시지는 6자유도(6-DOF)를 가진 로봇팔의 각 관절의 위치를 실시간으로 업데이트하는 데이터 구조로 정의된다.     |
| **Package**  | ROS 소프트웨어 구성 요소들의 집합 | 로봇팔 제어를 위한 패키지에는 "arm_control_package"라는 이름으로, 관절 제어 노드, 물체 인식 노드, 메시지 정의 파일, 설정 파일 등이 포함될 수 있다.     |
| **Launch File** | 여러 노드를 한 번에 실행하거나 ROS 시스템을 설정하는 데 사용되는 XML 형식의 스크립트 파일 | 로봇팔의 모든 관절 제어와 카메라를 통한 물체 인식 노드를 동시에 실행하도록 설정할 수 있다.     |
| **Transform**   | 좌표계 간의 변환을 관리하는 도구 | 카메라에서 인식한 물체의 위치를 로봇팔이 움직이는 좌표계로 변환해야 하는 경우, TF를 사용하여 카메라와 로봇팔 사이의 상대적인 좌표 변환을 설정할 수 있다.    |
| **ROS Master**  | ROS 시스템에서 모든 노드 간의 통신을 관리하는 중심 서버 역할 | "arm_controller" 노드와 "vision_processing" 노드는 각각 ROS 매스터에 자신을 등록하고, 이를 통해 서로 주고받을 데이터를 쉽게 찾고 통신을 설정할 수 있다. ROS Master는 네트워크 상에서 노드 간의 통신을 관리하는 허브 역할을 한다.     |
| **RViz**   | 로봇의 상태나 센서 데이터를 시각화하기 위한 도구 | 로봇팔이 특정한 물체를 잡으려고 움직이는 상황에서, 각 관절의 위치, 로봇팔의 말단이 가리키는 방향, 카메라로 인식한 물체의 위치 등을 3D 시각화하여 실시간으로 볼 수 있고, 이를 통해 로봇팔의 동작을 직관적으로 파악할 수 있다.     |
| **Gazebo** | ROS와 통합된 물리 기반 로봇 시뮬레이터 | 가상의 환경에서 물체를 잡는 작업을 시뮬레이션하여, 로봇팔의 각 관절 움직임, 중력, 충돌 등의 물리적 요소를 고려한 동작을 테스트하고, 실제 로봇팔을 사용하기 전에 문제를 사전에 발견할 수 있다.   |
