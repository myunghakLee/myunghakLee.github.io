...
실제 환경에서 인간의 행동을 예측하는 무척이나 어렵다. 우리는 single MAP trajectory prediction을 넘어 미래의 정확한 확률 분포를 얻는 것에 관심을 두었다. 따라서 우리는 궤적 분포 모드(modes of the trajectory distribution)에 해당하는 고정된 미래 상태 sequence 앵커 세트(fixed set of future state-sequence anchors)를 사용하는 multipath를 제시한다. 추론시 우리 모델은 앵커에 대한 이산 분포를 예측하고 각 앵커에 대해 불확실성과 함께 앵커 way-point에서 오프셋을 회귀하여 각 시간 단계에서 가우스 혼합을 산출한다.
...
우리는 다음 2가지 조건을 만족하는 네트워크를 찾는다.

1. 가능한 결과의 공간을 포괄하는 간결한 이산 궤적(discrete trajectories)의 집합
2. 모든 궤적의 가능성에 대한 폐쇄형 평가(closed-form evaluation)

위와 같은 2가지 속성은 crucial planning use-cases[[1\]](#_ftn1)에서 효율적인 추론(efficient reasoning)을 가능하도록 한다. 예를 들어 이산 궤적 가설(ex: yielding, following)에 대한 human-like reaction 및 시공간 영역에서 예상되는 충돌 위험과 같은 확률적 쿼리(probabilistic query)가 있다[[(2\]](#_msocom_2) .

이 두가지 속성은 모두 모델링 문제를 제시한다. Diversity와 coverage를 모두 달성하려는 모델은 종종 훈련중에 mode collapse로 고통받는 반면 가능한 궤적의 공간이 시간이 지남에 따라 기하 급수적으로 증가하기 때문에 다루기 쉬운 확률적 추론이 어렵다.

본 논문의 multipath 모델은 이와 같은 문제들을 해결 가능하게 한다. 이 방법은 고정된 궤적 앵커 세트를 사용한다. 이를 통해 확률적 불확실성을 계층 적으로 받아들일 수 있다.

첫째 의도 불확실성(intent uncertainty)은 agent가 수행하려는 작업의 불확실성을 포착하고 앵커 궤적 세트에 대한 분포로 인코딩 된다.

둘째 의도가 주어지면 제어 불확실성은 그것을 어떻게 달성할 수 있는지에 대한 우리의 불확실성을 나타낸다.
...
우리의 모델은 과거 접근 방식(MAP 궤적, 생성모델을 통해 가중치가 적용되지 않는 샘플 세트)들과 대조된다. 자율 주행 차량과 같은 실제 어플리케이션과 관련하여 샘플 기반 방법에는 여러 단점이 있다.

1. 안전에 중요한 시스템의 비결정성
2. 근사 오류 처리 불량(보행자가 무단횡단을 할 가능성을 알기 위해 몇 개의 샘플을 그려야 하나?)
3. 시공간 영역에 대한 기대치를 계산하는 것과 같은 관련 query에 대해 확률적 추론을 수행하는 쉬운 방법이 없다. 

우리는 우리의 모델이 synthetic 혹은 real 양쪽 모두에서 더 좋은 결과를 얻을 수 있음을 경험적으로 보여준다. 우리는 unimodal parametric distributions을 방출하는 모델보다 더 높은 likelihood를 달성하여 실제 데이터에서 다중 앵커의 중요성을 보였다. 또한 sample-set metric에서 훨씬 적은 수의 샘플로 미래를 더 잘 설명하는 anchor 당 MAP trajectory weight set을 사용하여 샘플링 기반 방법과 비교해 보았다.
...
**Inferring a diverse weighted set of test-time trajectories.** 본 논문의 모델을 사용하면 test time에 표준 샘플링 기술을 피하고 추가 계산 없이 K궤적의 weighted set을 얻을 수 있다: 각 K 앵커 모드에서 MAP 궤적 추정값을 취하고 앵커 π(ak | x)에 대한 분포를 샘플 가중치(즉 중요도 샘플링, 현재의 path가 선택될 확률)로 고려한다. Metric 및 applications에서 평가를 위해 상위 κ < K 궤적 세트를 요구할 때 이러한 샘플 가중치에 따라 상위 κ를 반환한다.

**Input representation.** 우리는 다른 recent approaches를 따르고 동적 및 정적 장면 컨텍스트의 history를 top-down orthographic 관점에서 렌더링 된 데이터의 3차원 배열로 나타낸다. 처음 두 차원은 하향식 이미지의 공간 위치를 나타낸다. 깊이 차원의 채널은 고정된 수의 이전 time-step의 정적 및 시간에 따라 변하는 (동적) content를 보유한다. Agent observation은 각 time-step에 대해 하나의 채널인 방향 경계 상자 이진 이미지로 렌더링 된다. 신호등 상태 및 도로의 정적 컨텍스트(차선 연결 및 유형, 정지선, 속도 제한 등)와 같은 기타 동적 컨텍스트는 추가 채널을 형성한다. 입력 내용이 dataset마다 다르므로 자세한 내용은 논문의 section 4를 참조하자. 이러한 top-down representation을 사용할 때의 중요한 이점은 agent간의 공간 관계 및 의미론적 도로 정보와 같은 상황 정보를 간단하게 표현할 수 있다는 것이다. B.4 섹션에서 행동 예측에 대한 이점을 경험적으로 강조한다.

**Neural network details.** 그림 1에서 보았듯이 우리는 먼저 전체 장면에 대한 특징 표현을 추출한 후 장면의 각 agent에 참여하여 agent 별 궤적 예측을 수행하는 jointly-train된 two-stage architecture를 설계하였다.
...
[25]에 이어 past state에 대한 선형 모델을 사용하여 단순한 등속 모델이 얼마나 성능이 나올지 확인한다. 과거에 관찰된 위치를 시간의 선형 함수로 fitting한다(x^t^ = [α^t^ + β, γ^t^ + δ] for t ≤ 0). 그리고 이 모델을 사용하여 미래의 위치를 나타낸다(x^1^, … , x^T^). 우리는 더 안좋은 결과에 대하여 고차 다항식을 사용하여 조사하였다.
...
(b)  : k=3 앵커가 있는 multipath는 intent와 uncertainty분포를 정확히 학습하여 높은 likelihood를 달성한다.

(c)   : single trajectory modeling은 경로가 갈라짐에 따라 증가하는 해당 location uncertainty와 함께 모든 trajectory에 대한 평균을 예측한다.

(d)  : kernel density estimate distrubution에 fit한 CVAE샘플

(e)  : Min-of K=5의 trajectory. 이 모델은 초기 가중치에 매우 민감하여 학습률이 4개인 5번의 시행에서 1,2개 정도의 활성 모드(active modes)로만 축소된다.





### **4.3 Toy experiment: 3-way intersection**
...
**Multipath example results**

한 장면에서 한 에이전트(focused agent)에 대한 결과를 한 열에 표시한다. 

Top : 모든 agent의 기록된 trajectory가 청록색으로 표시된다. Focused agent는 빨간색 원으로 강조표시된다.
